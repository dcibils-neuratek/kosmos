...
| `f21ae96`, 0.10.129-development - 0.10.128 with both chipset numbers put back and MSI allocation moved into `apic.c` - MEGA, same layout, for the ThinkCentre M700 | 512 MB, in a partition of its own | **Built and checked on 23 September, not yet booted there.** 32 checks under OVMF, `make test` green (33 suites, 6:50). Its numbering is 0.10.125's again - MSI from 20, links 16 to 19 - so it should be at least what booted here before, plus a `pci_enable` that says out loud when a device with an MSI capability ends up on a line. **What to look at**: `xhci: ... by interrupt` rather than `found by looking`, and no line mentioning `legacy line` |
