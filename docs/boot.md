...
| `90ec487`, 0.10.88-development - 0.10.87 plus the waits on the TSC and `/home` made fresh from `~/Kosmos/home` - MEGA, from `make MEGA=1 x86-usb-image` | **512 MB, in a partition of its own** - the first over 32 MB | **booted on 19 September**, to the desktop: Diego - "battery works!", "brightness keys work!", "snes controller works!". The first `/home` over 32 MB to boot on the ThinkPad |
