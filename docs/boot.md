...
| `401f3d6`, 0.10.128-development - 0.10.125 plus the whole display painted at 3440x1440 and an Intel Ethernet driver - MEGA, same layout, for the ThinkCentre M700 | 512 MB, in a partition of its own | **Built and checked on 23 September, not yet booted there.** 32 checks under OVMF, `make test` green (33 suites, 5:39), `make stress` clean (60 rounds, nothing leaked) and the benchmarks within tolerance - which is necessary and not sufficient, which is what this column is for. It is 0.10.125, the layout that booted on that machine, plus two revisions: 0.10.126, the framebuffer window that left the bottom of a 3440x1440 desktop unpainted, and 0.10.127, `e1000.c`. **What to look at**: the desktop filling the screen, and `neofetch` or `diagnose` for a `Network` row naming the I219 rather than saying it is not driven |
