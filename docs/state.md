...
Diego sent, and **drawn**: `docs/cafesa3d.html`, published for his phone at
...
1. **0.10.107** (`8c6e2ee`, committed, not pushed): the Deskbar's colour
   ...
   and a choice of 36, 44 or 52 (5v, 18.138). Waiting on Diego's yes to push
   both, and to push tags for 0.10.92 to 0.10.106 - "yes" was visible on his
   screen in a photograph, not said in the chat.
...
045E:028E on this Mac, and **under QEMU on the Mac every button arrived**
(`sudo sh tools/usbhost.sh 045e:028e 60`; macOS's own Xbox driver holds the
pad, so only root can pass it to QEMU). Xbox One after the stick.
...
**0.10.65-development supersedes it before it was tried**:
`build/x86_64/kosmos-usb-0.10.65-development.img`, `main` at `895aa3f` - the
same build plus the USB driver's wait on both of its endpoints (`70ff5e6`,
§18.71). Under OVMF on a snapshot: `log loader` clean with the stick against
the build the same, `/home` the named partition, `found at 0.68 s, by look 1`,
`diagnose` 14 KB, `diskbench /home` running, and `diskbench usb 0` at
764 MB/s and 10006 IOPS on the stick's blocks. **This is the one Diego writes**,
