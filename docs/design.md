...
**SMP.** Six of the seven steps in `docs/smp.md` are done, and the kernel is SMP-aware: per-CPU state through `TPIDR_EL1`, a spinlock, locked pools, one runqueue per processor with its own lock, per-endpoint locking in IPC, four processors each with their own exception vector, GIC redistributor, generic timer and idle thread, and an inter-processor interrupt worth a measured 25x on a cross-core wake. What is not switched on by default is the placement policy, though with `SMPWORK=4` work now spreads properly - 100% on all four cores under load. Getting there needed two fixes in the preemption path: `thread_tick` returned before `policy->tick` on every core but zero, and a cross-core wake flagged the waking core rather than the target. What still keeps it off by default is the display harness failing at its editor phase under placement. `docs/smp.md` has the measurement.
