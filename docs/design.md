...
**SMP.** Six of the seven steps in `docs/smp.md` are done, and the kernel is SMP-aware: per-CPU state through `TPIDR_EL1`, a spinlock, locked pools, one runqueue per processor with its own lock, per-endpoint locking in IPC, four processors each with their own exception vector, GIC redistributor, generic timer and idle thread, and an inter-processor interrupt worth a measured 25x on a cross-core wake. What is not switched on by default is the placement policy. The drivers were locked in 0.9.20, and with `SMPWORK=4` work still does not spread - only core zero preempts, because `thread_tick` returns before `policy->tick` everywhere else, and a cross-core wake flags the waking core rather than the target. `docs/smp.md` has the measurement.
