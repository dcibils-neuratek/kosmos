...
**Threads in a process.** There are none yet, and there is no syscall to make one. `SYS_SPAWN` makes a *process* - its own address space, its own capability table - which is the only unit of execution above a coroutine today. This section said otherwise for a long time and was simply wrong.

**They are coming, and the reason they were refused has stopped being true.** This paragraph said "that is not a gap to be filled later": a process has one `lua_State`, so two threads in it would need a lock around the whole interpreter and would take turns; what threads buy - overlapping the *waiting* - coroutines already give; and overlapping the *computing* "needs a second core rather than a second thread". That last clause was written when there was one core. There are four now, eight on the ThinkPad, and **a process is homed on exactly one of them** - so however much work one program has, it can have one core, and a video decoder, a software renderer, an audio project's mix and a planet simulation are each exactly that kind of program. Diego, 19 September 2026: "Why don't we add threading? To the kernel!", "Every modern os has multi threading as well as multi processing", "Threading will give us a lot of room for speed and super responsive ui and os".

What stays true is the first half: **one Lua interpreter cannot be shared between threads**, and locking it would buy nothing. So a thread is either C - a kit's decoder, mixer or rasteriser, which never touches Lua - or a Lua state of its own, talking to the others through channels rather than shared tables, which is LÖVE's `love.thread` and the same idea as messages between processes, inside one. Coroutines stay what a program uses to wait on many things; threads are for using more than one core. `roadmap.md` 4j has the steps, and `docs/threads.md` will be written, as `smp.md` was, before the kernel changes.

**SMP.** All seven steps in `docs/smp.md` are done, on both boards, and the kernel is SMP-aware: per-CPU state through `TPIDR_EL1` and the x86 `GS` base, a spinlock, locked pools, one runqueue per processor with its own lock, per-endpoint locking in IPC, each processor with its own exception vector, interrupt controller, timer and idle thread, and an inter-processor interrupt worth a measured 25x on a cross-core wake. x86 adds a TLB shootdown, which AArch64's broadcast invalidates make unnecessary. New threads spread across every processor that arrived by default since 0.10.22 - 100% on all four cores under load, and the ThinkPad's desktop across all eight - once the display harness passed with it on, which it did when `ipc_call` stopped losing a reply to a receiver on another core. `opt/kosmos/smp=N` narrows it, capped at the processors that arrived. `docs/smp.md` has the measurements.
...
That argument shaped the whole order in which it was built, and the shape is worth keeping: each step had to be *checkable* on its own. A parked core touches no shared structure, so it proves the bring-up without introducing a race; an idle thread on a secondary proves the core is alive without letting it near a shared queue; and the locks came only when there was a second scheduling core to contend for them. The step the plan put second - "take locks with one core, uncontended" - was skipped precisely because it is the one increment nothing can check.
