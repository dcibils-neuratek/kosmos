...
**SMP.** All seven steps in `docs/smp.md` are done, on both boards, and the kernel is SMP-aware: per-CPU state through `TPIDR_EL1` and the x86 `GS` base, a spinlock, locked pools, one runqueue per processor with its own lock, per-endpoint locking in IPC, each processor with its own exception vector, interrupt controller, timer and idle thread, and an inter-processor interrupt worth a measured 25x on a cross-core wake. x86 adds a TLB shootdown, which AArch64's broadcast invalidates make unnecessary. What is not switched on by default is the placement policy, though with `SMPWORK=4` work spreads - 100% on all four cores under load - and the display harness passes with it on since `ipc_call` stopped losing a reply to a receiver on another core. Switching it on by default waits for `make stress` with it on. `docs/smp.md` has the measurements.
