...
- [x] **Step 2. `SDL.h`.** `user/bin/apps/litexl/SDL.h` and
      `user/bin/apps/litexl/litexl_sdl.c`: the types, and the software surface -
...
- [x] **Step 4. The renderer.** `user/bin/apps/litexl/litexl_render.c`, on
...
**`user/bin/apps/doom/doom_kosmos.c` had already answered this** and its reasoning is
...
Nothing from SDL is linked, vendored or downloaded. `user/bin/apps/litexl/litexl_sdl.c`
...
So the build leaves `renderer.c` out and `user/bin/apps/litexl/litexl_render.c`
