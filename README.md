...
| Sep 2026 | **Every x86-64 vector has `ist = 0`, so a kernel stack overflow triple-faults the machine.** A fault from ring 0 stays on the stack that faulted, which is fine for a deliberate one and fatal for an overflow: the handler pushes onto the guard page and faults again. AArch64 survives it because the kernel runs on SP_EL0 and an exception switches to SP_EL1. Found by running the suite on the second board for the first time, and the fix is an interrupt stack table entry for #PF and #DF | arch/x86_64/trap.c |
