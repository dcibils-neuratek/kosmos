...
/*
 * `mfence` is the same choice on the other machine and for the same reason.
 *
 * The ordering `virtio_publish` asks for is free under TSO, so the shared
 * one is a compiler barrier. This is not asking for ordering: it is asking
 * for the samples to have *left* the store buffer before the device is told
 * to come and get them - and `mfence` is the one x86 instruction that
 * drains it. The stronger barrier costs a stall and buys certainty on a
 * 5.8 ms deadline; the paragraph above says why that trade was made.
 */
...
bool hal_snd_init(void)
{
    ...
    while (virtio_open(VIRTIO_ID_SOUND, from, &snd.dev)) {
         ...
         */
        virtio_enable_interrupt(&snd.dev);
